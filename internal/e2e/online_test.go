...
// TestVerifyOnlineParity verifies a legacy "online" commit - one whose Rekor
// entry is keyed on the commit SHA rather than embedded in the signature -
// through both the legacy verification path and the experimental sigstore-go
// path (cfg.EnableSigstoreGo), and asserts they agree on the signing certificate
// and Rekor log entry. This exercises the verifyOnline path, which fetches the
// commit-SHA entry via online search and verifies the assembled bundle with
// sigstore-go.
func TestVerifyOnlineParity(t *testing.T) {
	...
	// Legacy path: git.Verify + online commit-SHA Rekor lookup.
	legacyCfg := &config.Config{
		Fulcio: "https://fulcio.sigstore.dev",
		Rekor:  "https://rekor.sigstore.dev",
	}
	legacyVerifier, err := gitsign.NewVerifierWithCosignOpts(ctx, legacyCfg, nil)
	if err != nil {
	...
	// sigstore-go path: verifyBundle signals no embedded entry, Verify falls
	// through to verifyOnline.
	...
	if err != nil {
		t.Fatalf("online bundle verify: %v", err)
	}

	// The sigstore-go path must populate the bundle it verified.
	if bundle.Bundle == nil {
		t.Error("online bundle path did not populate VerificationSummary.Bundle")
	}
